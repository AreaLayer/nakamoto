...
use super::{
    chan, network::Network, output::message, traits, BlockHash, BlockHeader, Command, Config,
...
fn test_idle_disconnect() {
    ...
    peer.outputs()
        .find(|o| matches!(o, Io::Disconnect(addr, traits::DisconnectReason::Protocol(DisconnectReason::PeerTimeout("ping"))) if addr == &remote))
...
fn test_bad_magic() {
    ...
    peer.outputs()
        .find(|o| matches!(o, Io::Disconnect(addr, traits::DisconnectReason::Protocol(DisconnectReason::PeerMagic(999))) if addr == &remote))
...
fn test_handshake_version_timeout() {
    ...
    for link in &[Link::Outbound, Link::Inbound] {
        ...
        peer.outputs()
            .find(|o| {
                matches!(o, Io::Disconnect(a, traits::DisconnectReason::Protocol(
...
fn test_handshake_verack_timeout() {
    ...
    for link in &[Link::Outbound, Link::Inbound] {
        ...
        peer.outputs()
            .find(|o| {
                matches!(dbg!(o), Io::Disconnect(a, traits::DisconnectReason::Protocol(
                    DisconnectReason::PeerTimeout("handshake")
                )) if a == &remote.addr)
            })
