...
impl<U: Handshake + Wakeup + Connect + Disconnect + Events, C: Clock> PeerManager<U, C> {
    ...
    pub fn peer_disconnected<A: AddressSource>(
        ...
        addr: &net::SocketAddr,
        addrs: &mut A,
        reason: traits::DisconnectReason<DisconnectReason>,
    ) {
        let local_time = self.clock.local_time();
        ...
        if self.is_disconnecting(addr) || self.is_connected(addr) {
            ...
            if let traits::DisconnectReason::PeerConnectionError(err) = reason {
...
mod tests {
    ...
    fn test_persistent_client_reconnect() {
        ...
        peermgr.peer_disconnected(
            &remote,
            &mut addrs,
            traits::DisconnectReason::Protocol(DisconnectReason::PeerTimeout("")),
        );
    ...
    fn test_disconnects() {
        ...
        let reason: traits::DisconnectReason<DisconnectReason> =
